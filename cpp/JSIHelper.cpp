...
jsi::Value createSequelQueryExecutionResult(jsi::Runtime &rt, SQLiteOPResult status, jsi::Array &results, vector<QuickColumnMetadata> *metadata)
{
  ...
  // Converting row results into objects
    ...
    size_t row_count = results.size(rt);
  auto rows = jsi::Object(rt);
    if (row_count > 0) {
  // auto arr = r
  //   auto data = results->data()
  // auto row_array = jsi::Array(rt, row_count);
  // for (int i = 0; i < row_count; i++) {
//        auto row = results->getV
  //   row_array.setValueAtIndex(rt, i, move(row));
  // }
  ...
  rows.setProperty(rt, "length", jsi::Value((int)row_count));
